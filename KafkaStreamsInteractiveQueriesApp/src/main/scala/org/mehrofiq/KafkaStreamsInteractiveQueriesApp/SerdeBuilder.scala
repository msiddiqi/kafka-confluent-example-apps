package org.mehrofiq.KafkaStreamsInteractiveQueriesApp
...
trait SerdeBuilder {
  def specificAvroSerde[T <: SpecificRecord](isKey: Boolean) : SpecificAvroSerde[T]
  def genericAvroSerde(isKey: Boolean) : GenericAvroSerde
}
